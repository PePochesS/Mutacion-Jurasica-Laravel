...
@section('content')
  ...
  <div class="flash flash-success" id="flash-status">
    ...
    <button class="flash-close" type="button" aria-label="Cerrar" onclick="
...
@endif
    ...
    @if($game)
      {{-- Vista de una partida específica --}}
        ...
        @if($winner)
          <div class="menu-btn-container" style="margin-top:28px">
            <div class="back-menu" style="cursor:default">
              Ganador: Jugador {{ $winner->player_number }} — {{ $winner->points }} pts
            </div>
          </div>
        @endif
      </section>
    ...
    @else
      {{-- Listado de partidas finalizadas con filtro --}}
      <section class="scoreboard">
        <form method="GET" class="mb-3" style="display:flex; gap:10px; align-items:center">
          <label class="me-2">Filtrar por jugadores:</label>
        ...
        <ul class="score-grid">
          @forelse($games as $g)
            ...
            <li class="score-card" data-rank="0" style="grid-template-columns: 1fr auto">
              <a href="{{ route('ranking', ['game'=>$g->id]) }}" class="who" style="text-decoration:none; color:inherit">
                <h3>Partida #{{ $g->id }} — {{ $g->player_count }} jugadores</h3>
              </a>
              @if($top)
          ...
          @empty
            <li class="score-card" data-rank="0">
              <div class="who"><h3>No hay partidas finalizadas para mostrar.</h3></div>
            </li>
          @endforelse
        </ul>
      </section>
